...
{    
    public unsafe class IFProcessor : IIQProcessor
    {
        ...
        {
            e.Cancel = true;
            _control.Frequency = e.Frequency - (int) GetFilterOffset();
        }
        ...
        public bool Enabled
        {
            get { return _spectrumAnalyzer.Visible; }
            set { _spectrumAnalyzer.Visible = value; }
        }
        ...
        {
            if (_control.IsPlaying && _spectrumAnalyzer.Visible)
            {
                var totalBins = _fftBins * (double) _spectrumAnalyzer.SpectrumWidth / _sampleRate;
                var offsetBins = _fftBins * GetFilterOffset() / _sampleRate;
                var bins = Math.Min(_fftBins, totalBins);
                var ptr = _fftSpectrumPtr + (int) Math.Ceiling(offsetBins + (_fftBins - bins) * 0.5);
                _spectrumAnalyzer.Render(ptr, (int) bins);
        ...
        private double GetFilterOffset()
        {
            switch (_control.DetectorType)
            {
                case DetectorType.USB:
                    return _control.FilterBandwidth * 0.5;
        ...
        {
            _spectrumAnalyzer.SpectrumWidth = (int) Math.Max(Math.Min(_sampleRate, _control.FilterBandwidth * 1.5), 300);
            if (overrideFilter)
            {
                _spectrumAnalyzer.FilterBandwidth = _control.FilterBandwidth;
                _spectrumAnalyzer.FilterOffset = (int) GetFilterOffset();
                _filterOffset = _spectrumAnalyzer.FilterOffset;
                _filterbandwidth = _spectrumAnalyzer.FilterBandwidth;
                _updateFilter = true;
            }
            
            _spectrumAnalyzer.CenterFrequency = _control.Frequency + (int) GetFilterOffset();
            _spectrumAnalyzer.Frequency = _control.Frequency;
